...
exports.getInventryById=async(req, res)=>{
    try {
        const {id}=req.params;
        const getallbyid=await Inventry.findAll({where:{id}  
        });
        res.status(200).json(getallbyid);
    } catch (error) {
        res.status(500).json({message: "Faild to Get Inventry data by ID", error: error.message});
    }
}
